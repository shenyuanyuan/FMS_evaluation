...
log_file = "log/front_view_fms_evaluation_fcn.txt"
...
class Model(nn.Module):
    def __init__(self, net_params):
        super().__init__()
        self.model = BlockFCN(net_params['seq_len'], net_params['channel_params'], net_params['kernel_params'])
        self.model = self.model.cuda()
        self.fc = nn.Linear(net_params['channel_params'][-1], net_params['num_classes'])
...
if __name__ == '__main__':
    ...
    NUM_OF_TESTS = 10
    net_params = {'channel_params':[128, 64, 128, 64], 'kernel_params':[7, 5, 3], 'seq_len': 483, 'num_classes': 3}
    ...
    for i in range(NUM_OF_TESTS):
        x_train, x_test, y_train, y_test, z_train, z_test = train_test_split(data, labels, class_labels, test_size=0.3,
                                                                             random_state=0)
        # 建立训练集的dataloader
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for index, epoch_data in enumerate(train_loader):
                ...
                logger.info(
                    'Epoch: {:d}, Batch_index:{:d},  Loss: {:0.5f},   Acc: {:0.3f}%'.format(epoch, index, batch_loss,
                                                                                            batch_right_num / len(
                                                                                                predict) * 100))
            logger.info('Epoch: {:d},   Loss: {:0.5f},   Acc: {:0.3f}%'.format(epoch, train_loss,
                                                                               total_right_num / train_num * 100))
            ...
            logger.info('Test Epoch: {:d}, micro f1: {:0.3f}, macro f1: {:0.3f}, kappa: {:0.3f}'.format(epoch, micro_f1,
                                                                                                        macro_f1,
                                                                                                        kappa_score))
            cm = confusion_matrix(true_labels, predict_labels)
            logger.info(cm)
            val_losses.append(val_loss)
            final_res1.append(micro_f1)
            final_res2.append(macro_f1)
            final_res3.append(kappa_score)
        ...
        for c in range(num_classes):
            ...
            logger.info(
                'Class i: {:d}, Micro F1: {:0.3f}, macro F1: {:0.3f}, kappa: {:0.3f}'.format(c, micro_f1, macro_f1,
                                                                                             kappa_score))
