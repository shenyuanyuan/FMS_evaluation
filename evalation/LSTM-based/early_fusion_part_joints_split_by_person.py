...
log_file = r"log\multi_view_early_fusion_lstm_atten_with_person.txt"
...
class Atten(nn.Module):
    def __init__(self, size):
        ...
        self.weights = torch.nn.Parameter(torch.randn([1, size[0], size[1]]))
        
    def forward(self, data):
        weights = self.weights.repeat(data.shape[0], 1, 1)
...
if __name__ == '__main__':
    ...
    NUM_EPOCHS = 100
    NUM_OF_TESTS = 10
    net_params = {'channel_params':[2*joint_dim*valid_joints_num, 32, 64], 'seq_len': 545, 'num_classes': 3}
    ...
    for i in range(NUM_OF_TESTS):
        ...
        atten = Atten([net_params['seq_len'], 176])
        atten.cuda()
        ...
        start_lr = 0.005  #学习率
        learning_rate = 0.001
        optimizer = torch.optim.Adam([{'params': model.parameters()}, {'params': atten.parameters()}], lr = learning_rate)
        # Train
