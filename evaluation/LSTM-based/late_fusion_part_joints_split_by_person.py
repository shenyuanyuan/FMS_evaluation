...
log_file = r"log\multi_view_late_fusion_lstm_atten_with_person.txt"
...
class RNN(nn.Module):
    ...
    def forward(self,x):
        ...
        output = weight_r_out.reshape(len(weight_r_out), -1)
        return output
...
if __name__ == '__main__':
    ...
    for i in range(NUM_OF_TESTS):
        ...
        #model = BlockFCN(net_params['seq_len'], net_params['channel_params'], net_params['kernel_params'])
        model1 = RNN(net_params)
        model1.cuda()
        model2 = RNN(net_params)
        model2.cuda()
        fc = nn.Linear(2*2*net_params['channel_params'][1], net_params['num_classes'])
        fc.cuda()
        criterion = nn.CrossEntropyLoss()
        start_lr = 0.005  #学习率
        learning_rate = 0.001
        optimizer = torch.optim.Adam([{'params': model1.parameters()}, {'params': model2.parameters()}, {'params': fc.parameters()}], lr = learning_rate)
        # Train
        for epoch in range(NUM_EPOCHS):
            ...
            for index, epoch_data in enumerate(train_loader):
                ...
                front_batch_feas = batch_feas[:, :, :front_dim]
                side_batch_feas = batch_feas[:, :, front_dim:]
                
                front_output = model1(front_batch_feas)
                side_output = model2(side_batch_feas)
                output = torch.cat((front_output, side_output), dim = 1)
                output = fc(output)
