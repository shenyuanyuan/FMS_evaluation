...
from late_fusion_st_gcn import *

#(N, in_channels, T_{in}, V_{in}, M_{in})
log_file = "log/late_fusion_st_gcn_split_by_person.txt"
...
if __name__ == '__main__':
    ...
    for i in range(NUM_OF_TESTS):
        ...
        model1 = Model(joint_dim, num_class, graph_args, edge_importance_weighting=True)
        model1 = model1.to(device)
        model2 = Model(joint_dim, num_class, graph_args, edge_importance_weighting=True)
        model2 = model2.to(device)
        fc = nn.Conv2d(64*2, num_class, kernel_size=1)
        fc = fc.to(device)
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for index, epoch_data in enumerate(train_loader):
                ...
                front_batch_feas = batch_feas[:, :3]
                side_batch_feas = batch_feas[:, 3:]
                output1 = model1(front_batch_feas)
                output2 = model2(side_batch_feas)
