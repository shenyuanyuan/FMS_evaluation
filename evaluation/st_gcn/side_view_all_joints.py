...
from simple_st_gcn import *
from sklearn.metrics import cohen_kappa_score

#(N, in_channels, T_{in}, V_{in}, M_{in})
log_file = "log/side_view_st_gcn_joint_position.txt"
...
if __name__ == '__main__':
    ...
    data = side_data.reshape([side_data.shape[0], side_data.shape[1], valid_joints_num, joint_dim, 1])
    num_class = 3
    ...
    #logger.info('micro f1 accuracy: ')
    #logger.info(final_micro_f1_results)
    #logger.info('macro f1 result: ')
    #logger.info(final_macro_f1_results)
    #logger.info('kappa result: ')
    #logger.info(final_kappa_results)
    #logger.info('record test acc in each epoch: ')
    #logger.info(total_test_acc_0)
    #logger.info(total_test_acc_1)
