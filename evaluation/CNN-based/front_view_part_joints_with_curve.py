...
log_file = "log/front_view_cnn_with_curve.txt"
...
class Model(nn.Module):
    def __init__(self, net_params):
        super().__init__()
        self.model1 = BlockFCN(net_params['seq_len'], net_params['channel_params'], net_params['kernel_params_1'], 1)
        self.model1 = self.model1.cuda()
        self.model2 = BlockFCN(net_params['seq_len'], net_params['channel_params'], net_params['kernel_params_2'], 2)
        self.model2 = self.model2.cuda()
        self.model3 = BlockFCN(net_params['seq_len'], net_params['channel_params'], net_params['kernel_params_3'], 4)
        self.model3 = self.model3.cuda()
        self.model4 = BlockFCN(net_params['seq_len'], net_params['channel_params'], net_params['kernel_params_4'], 8)
        self.model4 = self.model4.cuda()
        self.fc = nn.Linear(4*net_params['channel_params'][-1], net_params['num_classes'])
        self.fc = self.fc.cuda()

    def forward(self, x):
        _, output_1 = self.model1(x)
        _, output_2 = self.model2(x)
        _, output_3 = self.model3(x)
        _, output_4 = self.model4(x)
        output = self.fc(torch.cat((output_1.squeeze(), output_2.squeeze(), output_3.squeeze(), output_4.squeeze()), dim = 1))
        return output
    ...
    for i in range(NUM_OF_TESTS):
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for index, epoch_data in enumerate(train_loader):
                ...
                train_loss_epoch = train_loss_epoch + loss.sum().item()
            ...
            train_losses.append(1.0*train_loss_epoch/train_num)
            ...
            for epoch_data in test_loader:
                ...
                val_loss = val_loss + criterion(output, batch_labels).sum().item()
            # use f1 
            ...
            logger.info(cm)
            val_losses.append(1.0*val_loss/test_num)
