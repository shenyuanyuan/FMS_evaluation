...
log_file = r"log\front_view_cnn_multi_scale_1248_dilation.txt"
...
class BlockFCNConv(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, dilation, momentum=0.99, epsilon=0.001, squeeze=False):
        ...
        self.relu = nn.ReLU()
    def forward(self, x):
        # input (batch_size, num_variables, time_steps)
        x = self.conv(x)
        x = self.batch_norm(x)
        # same shape as input
        y = self.relu(x)
...
if __name__ == '__main__':
    ...
    NUM_OF_TESTS = 10
    net_params = {'channel_params':[joint_dim*valid_joints_num, 64, 64], 'kernel_params_1':[3, 3], 'kernel_params_2':[3, 3], 'kernel_params_3':[3, 3], 'kernel_params_4':[3, 3], 'seq_len': 545, 'num_classes': 3}
    ...
    for i in range(file_num):
        ...
        #sample_indexes = get_samples_indexes(len(front_load_list), net_params['seq_len'])
        #front_load_list = [front_load_list[i] for i in sample_indexes]
        ...
        #sample_indexes = get_samples_indexes(len(side_load_list), net_params['seq_len'])
        #side_load_list = [side_load_list[i] for i in sample_indexes]
